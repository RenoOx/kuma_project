...
async function main(): Promise<void> {
  ...
  try {
    const [business] = await db
      .insert(businesses)
      .values({ name: 'Barbería Test LLM', whatsappNumber: '+51900000888' })
      .returning()
    ...
    const userText = 'Hola, cuánto sale un corte?'
    ...
    logger.info({ userText }, 'asking the LLM')
    ...
    const { content, tokensInput, tokensOutput } = replyResult.data
    const costUsd =
      (tokensInput / 1_000_000) * PRICE_PER_M_INPUT_USD +
      (tokensOutput / 1_000_000) * PRICE_PER_M_OUTPUT_USD

    logger.info({ reply: content }, 'LLM responded')
    logger.info(
      {
        tokensInput,
        tokensOutput,
