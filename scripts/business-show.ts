...
import { type BusinessSettings, businessSettingsSchema } from '@/modules/business/business.settings.js'
...
function showBusiness(business: Business, promptState: string | null): void {
  ...
  for (const id of nodes) {
    ...
    out(`  edgeCases: ${mark('edgeCases')} (${(override.edgeCases ?? blueprint.node.edgeCases).length})`)
    out(`  example:   ${mark('example')}`)
    if (override.extraInstructions?.trim()) out(`  extra:     "${override.extraInstructions.trim()}"`)
